...
class Scratch3LooksBlocks {
    ...
    getPrimitives () {
        return {
            ...
            looks_switchanimationto: this.switchAnimation,
            looks_switchbackdropto: this.switchBackdrop,
            looks_switchbackdroptoandwait: this.switchBackdropAndWait,
            looks_nextcostume: this.nextCostume,
            looks_nextanimation:this.nextAnimation,
    ...
    _setAnimation (target, requestedAnimation) {
        const anims =  target.getAnimations();
        if (typeof requestedAnimation === 'number') {
            if(requestedAnimation > anims.length){
                requestedAnimation = 0;
            }
            target.setAnimation(requestedAnimation);
        }else{
            for(let i = 0, l = anims.length; i < l; i++){
                if(anims[i].name === requestedAnimation){
                    target.setAnimation(i);
                    break;
                }
    ...
    switchAnimation (args, util){
        this._setAnimation(util.target, args.ANIMATION);
    }
    ...
    nextAnimation (args, util) {
        this._setAnimation(
            util.target, util.target.currentAnimation + 1
        );
    }
