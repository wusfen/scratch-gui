...
class Scratch3EventBlocks {
    ...
    broadcastDispatchEvent (args, util) {
        // console.log('args.BROADCAST_OPTION.name:', args.BROADCAST_OPTION.name);
        if (/^\s*(#答案正确\*|练习完成继续学习)\s*$/.test(args.BROADCAST_OPTION.name)){
            window.dispatchEvent(new Event('codeRunningRight'));
        }
    }

    broadcast (args, util) {
        this.broadcastDispatchEvent(...arguments);
