...
class Mouse {
    ...
    postData (data) {
        ...
        if(('ontouchstart' in window) && stageSize.stageRotate == 270){
            let tmp = data.y;
            data.y = data.x;
            data.x = data.canvasHeight - tmp;
            tmp = data.canvasHeight;
            data.canvasHeight = data.canvasWidth;
            data.canvasWidth = tmp;
        }
        if (data.x) {
            this._clientX = data.x;
            this._scratchX = Math.round(MathUtil.clamp(
                data.canvasWidth * ((data.x / data.canvasWidth) - 0.5),
                -(data.canvasWidth / 2),
                (data.canvasWidth / 2)
            ));
        }
        if (data.y) {
            ...
            this._scratchY = Math.round(MathUtil.clamp(
                -data.canvasHeight * ((data.y / data.canvasHeight) - 0.5),
                -(data.canvasHeight / 2),
                (data.canvasHeight / 2)
            ));
        }
        // fixed: css 修改舞台显示尺寸后鼠标对于舞台的坐标不对
        this._scratchX = this._scratchX / stageSize.stageScale;
        this._scratchY = this._scratchY / stageSize.stageScale;
