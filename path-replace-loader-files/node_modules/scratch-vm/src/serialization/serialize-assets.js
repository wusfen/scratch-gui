...
const serializeAssets = function (runtime, assetType, optTargetId) {
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        if(currTarget.isSkelSprite){
            for (let [key, value] of currTarget.sprite.cacheData) {
                assetDescs.push({fileName: key, fileContent: value});
            }
            const currAssets = currTarget.sprite[assetType];
            if(currAssets.length > 0){
                const asset = currAssets[0].asset;
                assetDescs.push({
