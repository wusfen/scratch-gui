...
const SkeletonSprite = require('@/spine/SkeletonSprite').default;
const {deserializeSkeletonAsset} = require('@/spine/skel-uploader');
...
const {loadCostume, loadAsset} = require('../import/load-costume.js');
...
const serializeTarget = function (target, extensions) {
    ...
    if (obj.isStage) { // Only the stage should have these properties
        ...
        obj.isSkelSprite = target.isSkelSprite;
    }
...
const parseScratchAssets = function (object, runtime, zip) {
    ...
    assets.costumePromises = (object.costumes || []).map(costumeSource => {
        ...
        return deserializeCostume(costume, runtime, zip)
            .then(() => {return costume.asset?costume:loadAsset(costumeMd5Ext, costume, runtime)});//加载aaset
            //.then(() => costume);//不加载外观
...
const parseScratchObject = function (object, runtime, extensions, zip, assets) {
    ...
    let sprite;
    if (object.hasOwnProperty('isSkelSprite')){
        sprite = new SkeletonSprite(blocks, runtime);
    }else{
        // @todo: For now, load all Scratch objects (stage/sprites) as a Sprite.
        sprite = new Sprite(blocks, runtime);
    }
    ...
    const finishFun = ()=>{
        const costume = sprite.costumes[target.currentCostume];
        if(costume.skinId){
            return target;
        }
        return loadCostume(costume.md5, costume, runtime).then(() => target);
    }
    return Promise.all(costumePromises.concat(soundPromises)).then(()=>{
        if(target.isSkelSprite){
            return deserializeSkeletonAsset(target, zip).then(()=>finishFun());
        }
        return finishFun();
    });
};
