...
class RenderedTarget extends Target {
    ...
    getSkinSize(){
        ...
        if(costumeSize[0] === 0){//没有外观就找一个有的
            costumeSize = this._getOneSkinSize();
        }
        return costumeSize;
    }
    ...
    setTileOffset(x, y){
        if(this.isStage){
            ...
            if (this.renderer) {
                let costumeSize = this.getSkinSize();
                if(costumeSize[0] !== 0){
                    x = x % costumeSize[0];
                    y = y % costumeSize[1];
                }
                this.renderer.updateDrawableTileOffset(this.drawableID, x, y);
    ...
    _getOneSkinSize(){
        if(this.renderer){
            for(let i = 0, l = this.sprite.costumes.length; i < l; i++){
                let skinId = this.sprite.costumes[i].skinId;
                let size = this.renderer.getSkinSize(skinId);
    ...
    _updateDrawableSkin(drawableID, costume){
        this.renderer.updateDrawableSkinId(drawableID, costume.skinId);
    }
    ...
    setCostume (index) {
        ...
        if (this.renderer) {
            ...
            if(null == costume.skinId){
                const md5ext = `${costume.assetId}.${costume.dataFormat}`;
                loadCostume(md5ext, costume, this.runtime).then(() => {
                    this._updateDrawableSkin(this.drawableID, costume);
                    if (this.visible) {
                        this.emit(RenderedTarget.EVENT_TARGET_VISUAL_CHANGE, this);
                        this.runtime.requestRedraw();
                    }
    ...
    updateAllDrawableProperties () {
        if (this.renderer) {
            ...
            this.renderer.updateDrawableVisible(this.drawableID, this.visible);
            if(this.isStage){
                this.renderer.updateDrawableTileOffset(this.drawableID, this.tileX, this.tileY);
            }
            const costume = this.getCostumes()[this.currentCostume];
            if(null == costume.skinId){
                const md5ext = `${costume.assetId}.${costume.dataFormat}`;
            ...
            if (this.visible && null !== costume.skinId && -1 !== costume.skinId) {
                this.emit(RenderedTarget.EVENT_TARGET_VISUAL_CHANGE, this);
                this.runtime.requestRedraw();
            }
        }
    ...
    saveState(){//保存角色的状态
        if(this._x === undefined){
            ...
            this._draggable = this.draggable;
            this._currentCostume = this.currentCostume;
            this._visible = this.visible;
            this._rotationStyle = this.rotationStyle;
            if(this.isStage){
                this._tileX = this.tileX;
                this._tileY = this.tileY;
            }
        }
    }
    ...
    restoreState(){//恢复角色的状态
        if(this._x !== undefined){
            ...
            this.currentCostume = this._currentCostume;
            this.rotationStyle = this._rotationStyle;
            if(this.isStage){
                this.tileX = this._tileX;
                this.tileY = this._tileY;
            }
            ...
            this._currentCostume = undefined;
            this._visible = undefined;
            this._rotationStyle = undefined;
    ...
    get hidden(){
        let block;
        for(var key in this.blocks._blocks){
            block = this.blocks._blocks[key];
            if(!this._needHidden(block.fields, "VARIABLE")){
                if(!this._needHidden(block.fields, "BROADCAST_OPTION")){
                    if(this._needHidden(block.fields, "LIST")){
                        return true;
                    }else{
                        let mutation = block.mutation;
                        if(mutation && mutation.proccode && mutation.proccode.indexOf('#') === 0 && -1 !== mutation.proccode.indexOf('*')){
                            return true;
                        }
                    }
                }else{
