...
const {OrderedMap} = require('@/../node_modules/immutable');
...
class Runtime extends EventEmitter {
    ...
    static get STAGE_HEIGHT () {
        return window.STAGE_HEIGHT || 667;
    }
    ...
    // 运行刚开始状态
    static get PROJ_NEW_START () {
        return 'NEW_PROJ_START';
    }

    // 运行已开始状态
    static get PROJ_STARTED () {
        return 'PROJ_STARTED';
    }
    ...
    scanForPeripheral (extensionId) {
        if (this.peripheralExtensions[extensionId]) {
            if (window.bridge){
                window.bridge.emit('startWDCodeLink');
                setTimeout(() => {
    ...
    attachRenderer (renderer) {
        ...
        if (canvas){
            canvas.addEventListener('webglcontextlost', event => {
                this.renderer.webglContextLost();
            ...
            canvas.addEventListener("webglcontextrestored", () => {
                this.renderer.webglContextRestored();
                if (this._steppingInterval) return;
                if (this.currentStepTime){
    ...
    startHats (requestedHatOpcode,
        ...
        if (this._pro_run_stop === undefined){
            return [];
        }
        const instance = this;
        const newThreads = [];
    ...
    dispose () {
        ...
        this.renderer && this.renderer.destroySkins();// 内存泄露问题
    ...
    greenFlag () {
        this.stopAll();
        this._pro_run_stop = Runtime.PROJ_NEW_START;
    ...
    stopAll () {
        // Emit stop event to allow blocks to clean up any state.
        this._pro_run_stop = Runtime.PROJ_END;
    ...
    _emitProjectRunStatus (nonMonitorThreadCount) {
        if (this._nonMonitorThreadCount === 0 && nonMonitorThreadCount > 0 || this._pro_run_stop === Runtime.PROJ_NEW_START) {
            this.emit(Runtime.PROJECT_RUN_START);
            this._pro_run_stop = Runtime.PROJ_STARTED;
        }
        if (this._pro_run_stop === Runtime.PROJ_END) {
            this._pro_run_stop = undefined;
