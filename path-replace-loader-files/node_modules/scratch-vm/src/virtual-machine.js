...
class VirtualMachine extends EventEmitter {
    ...
    loadProject (input) {
        ...
        return validationPromise
            .then(validatedInput => {
                debugger
                window.originSb3List = validatedInput[0].targets.map(e => e.costumes) // 记录初始的资源列表
                    .flat()
                    .map(e => e.md5ext)
                    .concat(validatedInput[0].targets.map(e => e.sounds)
                        .flat()
                        .map(e => e.md5ext));
    ...
    deserializeProject (projectJSON, zip) {
        console.log("projectJSON, zip------", projectJSON, zip);
    ...
    addBackdrop (md5ext, backdropObject) {
        var stagex = this.runtime.getTargetForStage();
        console.log("🚀 ~ file: virtual-machine.js ~ line 959 ~ VirtualMachine ~ addBackdrop ~ stagex", stagex)
        return loadCostume(md5ext, backdropObject, this.runtime).then(() => {
            const stage = this.runtime.getTargetForStage();
            stage.addCostume(backdropObject);
            stage.setCostume(stage.getCostumes().length - 1);
            this.runtime.emitProjectChanged();
            console.log("🚀 ~ file: virtual-machine.js ~ line 977 ~ VirtualMachine ~ returnloadCostume ~ emitProjectChanged", this.runtime.emitProjectChanged)
        });
    }
    ...
    deleteSprite (targetId) {
        ...
        if (target) {
            ...
            for(var l = sprite.costumes_.length - 1; l >= 0; l --){
                this.renderer.destroySkin(sprite.costumes_[l].skinId);
            } 
