...
class ScratchLinkWebSocket {
    ...
    open () {
        ...
        const scratchLinkWs = params.get('scratchLinkWs') ||
            /vipthink/i.test(navigator.userAgent) ?
            'ws://127.0.0.1:20220' :
            'wss://device-manager.scratch.mit.edu:20110';
        console.log('scratchLinkWs:', scratchLinkWs);

        switch (this._type) {
        case 'BLE':
            this._ws = new WebSocket(`${scratchLinkWs}/scratch/ble`);
            break;
        case 'BT':
            this._ws = new WebSocket(`${scratchLinkWs}/scratch/bt`);
            break;
        default:
