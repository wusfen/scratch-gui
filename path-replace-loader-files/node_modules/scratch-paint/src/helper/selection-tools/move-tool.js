...
class MoveTool {
    ...
    onMouseDrag (event) {
        ...
        for (const item of this.selectedItems) {
            ...
            //不允许拖出边境
            const h_W = item.bounds.width / 2.0;
            const h_H = item.bounds.height / 2.0;
            if(item.position.x - h_W < actionBounds.left){
                item.position.x = actionBounds.left + h_W;
            }
            if(item.position.y - h_H < actionBounds.top){
                item.position.y = actionBounds.top + h_H;
            }
            if(item.position.x + h_W > actionBounds.right){
                item.position.x = actionBounds.right - h_W;
            }
            if(item.position.y + h_H > actionBounds.bottom){
                item.position.y = actionBounds.bottom - h_H;
            }
