...
class RenderWebGL extends EventEmitter {
    ...
    static _getContext (canvas) {
        return twgl.getWebGLContext(canvas, {alpha: false, stencil: true, antialias: true});
    }
    ...
    webglContextRestored(){
        const gl = this._gl = RenderWebGL._getContext(this._gl.canvas);
        this._createGeometry();
        this._shaderManager = new ShaderManager(gl);
        gl.disable(gl.DEPTH_TEST);
        gl.enable(gl.BLEND);
    ...
    webglContextLost(){
        const keys = Object.keys(this._allSkins);
        for(let i =0, l = keys.length; i < l; i++){
            this._allSkins[keys[i]].deleteTexture();
        }
    }
    ...
    destroySkins(){
        const keys = Object.keys(this._allSkins);
        for(let i =0, l = keys.length; i < l; i++){
            this._allSkins[keys[i]].dispose();
        }
        this._allSkins.length = 0;
    }
    ...
    draw () {
        ...
        if(this._deletePreSkins.length > 0){
            this._deletePreSkins.forEach(skin => skin.deleteTexture());
            this._deletePreSkins.length = 0;
        }
    }
    ...
    updateDrawableSkinId (drawableID, skinId) {
        ...
        if (!drawable) return;
        if(drawable.skin){
            if(drawable.skin.id === skinId){
                return;
            }
            this._deletePreSkins.push(drawable.skin);
            //drawable.skin.deleteTexture();
        }
        drawable.skin = this._allSkins[skinId];
        for(var l = this._deletePreSkins.length - 1; l >=0; l--){
            if(this._deletePreSkins[l].id === drawable.skin.id){
                this._deletePreSkins.splice(l, 1);
                break;
            }
        }
    }
    ...
    updateDrawableTileOffset(drawableID, x, y){
        const drawable = this._allDrawables[drawableID];
        if (!drawable) return;
        drawable.updateTileOffset(x, y);
    }
