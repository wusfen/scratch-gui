    ...
    return (assetId, data) => {
        if (cacheId !== assetId) {
            if (typeof btoa === 'undefined') {
                ...
                cacheId = assetId;
                cacheString = base64js.fromByteArray(data);
                return cacheString;
            } else {
...
class Asset {
    ...
    setThumbnailURI(img){
        ...
        canvas.height = img.naturalHeight * img.width / img.naturalWidth;
        if(canvas.height > img.height){
            canvas.width =  img.naturalWidth * img.height / img.naturalHeight;
            canvas.height = img.height;
        }
        ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
        this.thumbnailURI = canvas.toDataURL();
    }
